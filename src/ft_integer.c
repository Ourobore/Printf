...
void		integer_write_fill(t_flags f, t_type t, int sign, char *base)
{
	long int len_int;
	...
	len_int = get_length_int_base(t.i, base, &len_int);
	while (f.precision != -1 &&
			f.width > (len_int > f.precision ? len_int : f.precision) + sign)
	{
		write(1, " ", 1);
		f.width--;
		g_nb_carac++;
...
void		ft_integer(t_flags f, t_type t, char *base)
{
	...
	if (f.fill && !f.star && !f.left)
		integer_write_fill(f, t, sign, base);
	else if (f.left)
